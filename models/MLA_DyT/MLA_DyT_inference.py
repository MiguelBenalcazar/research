...
class MLA(nn.Module):
    ...
    def __init__(self, args: ModelArgs):
        ...
        if self.q_lora_rank == 0:
            ...
            self.q_norm = DyT(num_features = self.q_lora_rank, alpha_init_value=0.3)
        ...
        self.kv_norm = DyT(num_features =self.kv_lora_rank,  alpha_init_value=0.3)
...
class MLP(nn.Module):
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        """
        return self.w2(nn.functional.silu(self.w1(x)) * self.w3(x)) #Output=W_2(SiLU(W_1x) ⊙ (W_3x))

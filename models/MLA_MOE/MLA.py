...
class ModelArgs:
    ...
    Attributes:
        ...
        bias_update_speed (float): gamma to adjust the over-use of an expert
        bias_update_threshold (float): a threshold is set to control the over-use of an experts
    """
    ...
    bias_update_max_imbalance: float = 0.75
...
class Gate(nn.Module):
    ...
    Attributes:
        ...
        bias_update_speed (float): Hyperparameter gamma for bias adjustment.
    ...
    def __init__(self, args: ModelArgs):
        ...
        self.expert_scores = None  # To store raw scores for balance loss

        # Bias update
        self.bias_update_speed = getattr(args, 'bias_update_speed', 0.01)  # gamma
        self.bias_update_max_imbalance =  getattr(args, "bias_update_max_imbalance", 0.75)
    ...
    def forward(self, x: torch.Tensor) -> Tuple[torch.Tensor, torch.Tensor]:
        ...
        self.expert_scores = scores  # Store for balance loss

        # Auxiliary-Loss-Free Load Balancing: Add bias for routing only
        routing_scores = scores
        if self.bias is not None:
            routing_scores = scores + self.bias  # s_i,t + b_i

        # Grouping and Top-K_r routing (select top K_r groups)
        if self.n_groups > 1:
            # Reshape the affinity scores into groups: (batch_size, n_groups, experts_per_group)
            routing_scores = routing_scores.view(x.size(0), self.n_groups, -1)
          
            # Compute group scores by taking the maximum score per group
            group_scores = routing_scores.amax(dim=-1)
            
            # Select top-K_r groups
            group_indices = group_scores.topk(self.topk_groups, dim=-1)[1]  # Shape: (batch_size, topk_groups)
            # Create mask for unselected groups (True for unselected, False for selected)
            mask = torch.ones(x.size(0), self.n_groups, dtype=bool, device=routing_scores.device)
            mask.scatter_(1, group_indices, False)
            # Set scores of unselected groups to -inf
            routing_scores = torch.where(
                mask.unsqueeze(-1),
                torch.full_like(routing_scores, float("-inf")),
                routing_scores
            ).flatten(1)
        else:
            routing_scores = routing_scores  # No grouping needed

        # Select top-K experts (select top K experts across all selected groups)
        expert_indices = torch.topk(routing_scores, self.topk, dim=-1)[1]  # Shape: (batch_size, topk)
        weights = original_scores.gather(1, expert_indices)


        # Store expert assignments for load balancing
        self.expert_assignments = expert_indices.detach()  # Shape: (batch_size, topk)

        # Normalize weights if using sigmoid (softmax is already normalized)
        if self.score_func == "sigmoid":
            weights = weights / (weights.sum(dim=-1, keepdim=True) + 1e-6)  # Add epsilon for numerical stability
    ...
    def compute_balance_loss_before_alpha(self) -> torch.Tensor:
        """
        Computes the sequence-wise balance loss as per Equations (17)-(20).

        Returns:
            torch.Tensor: The balance loss.
        """
        if self.expert_scores is None or self.expert_assignments is None:
            return torch.tensor(0.0, device=self.weight.device, requires_grad=True)

        # Step 1: Compute s'_i,t (Equation 19)
        # s_i,t is self.expert_scores, shape (batch_size * seq_len, n_routed_experts)
        s_it = self.expert_scores
        if self.score_func == "sigmoid":
            s_prime_it = s_it / (s_it.sum(dim=-1, keepdim=True) + 1e-6)  # Normalize over all experts
        else:
            s_prime_it = s_it

        # Step 2: Compute P_i (Equation 20)
        # Average s'_i,t over all tokens
        T = s_prime_it.size(0)  # batch_size * seq_len
        P_i = s_prime_it.mean(dim=0)  # Shape (n_routed_experts,)

        # Step 3: Compute f_i (Equation 18)
        # Count how many times each expert is in the Top-K
        expert_counts = torch.bincount(
            self.expert_assignments.flatten(),
            minlength=self.n_routed_experts
        ).float()  # Shape (n_routed_experts,)
        K_r = self.topk
        f_i = (self.n_routed_experts / (K_r * T)) * expert_counts

        self.expert_scores = None # clean expert scores
        # Step 4: Compute the balance loss no alpha yet (Equation 17)
        return  (f_i * P_i).sum()
    ...
    def update_bias(self):
        """
        Updates the bias term based on expert load at the end of a training step.
        Decreases the bias by gamma for overloaded experts and increases it by gamma for underloaded experts.
        """
       
        if self.bias is None or self.expert_assignments is None:
            return  # No bias to update or no assignments recorded
        ...
        # Compute ideal load per expert
        batch_size = self.expert_assignments.size(0)
        total_assignments = batch_size * self.topk
        ideal_load =  max(total_assignments / self.n_routed_experts, 1e-6) # avoid multiplication by zero

        # Detect imbalance
        imbalance_ratio = expert_loads.max() / expert_loads.min().clamp(min=1e-6)
 

        # Determine overloaded and underloaded experts
        load_diff = expert_loads - ideal_load  # Positive if overloaded, negative if underloaded
        adjustment = -self.bias_update_speed * torch.tanh(load_diff / ideal_load)  # Smooth updates

        # Compute bias adjustment
        with torch.no_grad():
            if imbalance_ratio > self.bias_update_max_imbalance:
                self.bias.add_(adjustment)
          
        # Reset expert assignments after updating
...
class MoE(nn.Module):
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        counts = torch.bincount(indices.flatten(), minlength=self.n_routed_experts)[self.experts_start_idx:self.experts_end_idx].tolist()
...
class Block(nn.Module):
    ...
    def __init__(self, layer_id: int, args: ModelArgs):
        ...
        self.balance_complem_seq_wise_aux_loss = None
        self.expert_assigment = None
    ...
    def forward(self, x: torch.Tensor, start_pos: int, mask: Optional[torch.Tensor]) -> torch.Tensor:
        ...
        if isinstance(self.ffn, MoE):
            ...
            self.balance_complem_seq_wise_aux_loss = self.ffn.gate.compute_balance_loss_before_alpha()
        else:
            self.balance_complem_seq_wise_aux_loss = torch.tensor(0.0, device=x.device, requires_grad=True)
...
class Transformer(nn.Module):
    ...
    def get_complementary_sequence_wise_aux_for_loss(self):
        by_layer = []
        for layer in self.layers:
            by_layer.append(layer.balance_complem_seq_wise_aux_loss)
        return by_layer